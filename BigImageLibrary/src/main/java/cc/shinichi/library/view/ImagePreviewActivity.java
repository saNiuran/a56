...
public class ImagePreviewActivity extends AppCompatActivity implements Handler.Callback, View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            boolean isGranted = PermissionUtils.hasSelfPermissions(this, Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_EXTERNAL_STORAGE);
            if (isGranted) {
                downloadCurrentImg();
                return;
            }
...
//                    .setRationaleMsg("请授权APP存储权限，用于缓存APP使用过程的文件和图片；若被拒绝的话，会造成APP无法正常使用")
