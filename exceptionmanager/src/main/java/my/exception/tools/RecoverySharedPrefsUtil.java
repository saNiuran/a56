...
public class RecoverySharedPrefsUtil {
    ...
    public static void recordCrashData() {
        ...
        try {
            ...
            count = 0;
            time = 0L;
            my.exception.tools.RecoveryLog.e(e.getMessage());
        }
        ...
        my.exception.tools.RecoveryLog.e(data.toString());
        saveCrashData(data);
    }
    ...
    private static void saveCrashData(CrashData data) {
        if (data == null)
            return;
        my.exception.tools.SharedPreferencesCompat.newBuilder(Recovery.getInstance().getContext(), SHARED_PREFS_NAME)
    ...
    public static void clear() {
        my.exception.tools.SharedPreferencesCompat.clear(Recovery.getInstance().getContext(), SHARED_PREFS_NAME);
    }

    private static void put(String key, String value) {
        my.exception.tools.SharedPreferencesCompat.put(Recovery.getInstance().getContext(), SHARED_PREFS_NAME, key, value);
    }

    private static String get(String key, String defValue) {
        return my.exception.tools.SharedPreferencesCompat.get(Recovery.getInstance().getContext(), SHARED_PREFS_NAME, key, defValue);
    }
